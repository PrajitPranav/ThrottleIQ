// analytics_screen.dart — Expanded driving intelligence analytics dashboard.
...
class AnalyticsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (context, _) {
        ...
        if (trips.isEmpty) return _emptyAnalytics();
        ...
        // ── Behavior aggregates ───────────────────────────────────────────
        final double avgBrakeScore = trips.fold(0.0, (sum, t) => sum + t.brakeScore) / trips.length;
        final double avgAccelScore = trips.fold(0.0, (sum, t) => sum + t.accelScore) / trips.length;

        final int totalHarshBrakes = trips.fold(0, (sum, t) => sum + t.harshBrakingCount);
        final int totalAggAccel    = trips.fold(0, (sum, t) => sum + t.aggressiveAccelCount);
        final int totalSharpTurns  = trips.fold(0, (sum, t) => sum + t.sharpTurnCount);
        final int totalLeftTurns   = trips.fold(0, (sum, t) => sum + t.leftTurnCount);
        final int totalRightTurns  = trips.fold(0, (sum, t) => sum + t.rightTurnCount);
        ...
        for (var i = 0; i < 7; i++) {
          ...
          if (dayTrips.isNotEmpty) {
            weeklyData[i] = dayTrips.fold(0.0, (sum, t) => sum + t.distanceKm) / 50.0;
          }
        }
        ...
        return SingleChildScrollView(
          ...
          child: Column(children: [
            ...
            Text(
              DriveScoreEngine.rankLabel(avgScore.round()),
              style: GoogleFonts.inter(
                fontSize: 12, fontWeight: FontWeight.w500,
                letterSpacing: 1.0, color: const Color(0xFF5A7D65)),
            ),
            ...
            Padding(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceAround,
                children: [
                  _Ring(label: 'BRAKING',    score: avgBrakeScore, color: const Color(0xFF8F3232), size: 100, sw: 6),
                  _Ring(label: 'ACCEL',      score: avgAccelScore, color: const Color(0xFF9E653F), size: 100, sw: 6),
                ],
              ),
            ),
            const SizedBox(height: 20),
            ...
            Padding(
              ...
              child: Row(
                children: [
                  _behaviorChip(Icons.warning_amber_rounded, 'HARSH BRAKES', totalHarshBrakes.toString(), const Color(0xFF8F3232)),
                  const SizedBox(width: 10),
                  _behaviorChip(Icons.speed_rounded, 'HARD ACCELS', totalAggAccel.toString(), const Color(0xFF9E653F)),
                  const SizedBox(width: 10),
                  _behaviorChip(Icons.rotate_right_rounded, 'SHARP TURNS', totalSharpTurns.toString(), const Color(0xFF4F6B8F)),
                ],
              ),
            ),
            const SizedBox(height: 12),
            Padding(
              ...
              child: Row(
                children: [
                  _behaviorChip(Icons.turn_left_rounded,  'LEFT TURNS',  totalLeftTurns.toString(),  const Color(0xFF4F6B8F)),
                  const SizedBox(width: 10),
                  _behaviorChip(Icons.turn_right_rounded, 'RIGHT TURNS', totalRightTurns.toString(), const Color(0xFF4F6B8F)),
                  const SizedBox(width: 10),
                  _behaviorChip(Icons.route_rounded,      'TOTAL TRIPS', trips.length.toString(),    const Color(0xFF5A7D65)),
                ],
              ),
            ),
  ...
  // ── Behavior chip ──────────────────────────────────────────────────────────
  Widget _behaviorChip(IconData icon, String label, String value, Color color) {
    return Expanded(
      child: Container(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text('Complete a few trips to see your driving analysis',
  ...
  List<Widget> _generateInsights(List<Trip> trips) {
    ...
    final double avgMaxG = trips.fold(0.0, (s, t) => s + t.maxGForce) / trips.length;
    ...
    if (avgMaxG > 1.2) {
      insights.add(const _InsightCard(
        ...
        body: 'Your cornering forces are quite high. Consider smoother entries to maintain tire grip.',
        color: Color(0xFF8F3232),
      ));
    } else {
      insights.add(const _InsightCard(
        icon: Icons.check_circle_outline_rounded,
        title: 'Excellent Smoothness',
        body: 'Consistent G-Force management. Your driving style minimizes vehicle wear.',
        color: Color(0xFF5A7D65),
      ));
    }

    // Braking insight
    final int totalHarsh = trips.fold(0, (s, t) => s + t.harshBrakingCount);
    if (totalHarsh > trips.length * 2) {
      insights.add(const _InsightCard(
        icon: Icons.warning_amber_rounded,
        title: 'Braking Pattern',
        body: 'High harsh braking frequency detected. Anticipate stops earlier for smoother deceleration.',
        color: Color(0xFF9E653F),
      ));
    } else if (totalHarsh == 0) {
      insights.add(const _InsightCard(
        icon: Icons.check_circle_outline_rounded,
        title: 'Smooth Braking',
        body: 'Zero harsh braking events across all trips. Your anticipation skills are excellent.',
        color: Color(0xFF5A7D65),
      ));
    }

    // Acceleration insight
    final int totalAggAccel = trips.fold(0, (s, t) => s + t.aggressiveAccelCount);
    if (totalAggAccel > trips.length * 3) {
      insights.add(const _InsightCard(
        icon: Icons.speed_rounded,
        title: 'Aggressive Acceleration',
        body: 'Frequent hard acceleration detected. Gradual throttle inputs improve efficiency and tire life.',
        color: Color(0xFF9E653F),
      ));
    }

    // Turn insight
    final int totalSharpTurns = trips.fold(0, (s, t) => s + t.sharpTurnCount);
    if (totalSharpTurns > trips.length * 2) {
      insights.add(const _InsightCard(
        icon: Icons.rotate_right_rounded,
        title: 'Sharp Turn Frequency',
        body: 'Multiple sharp turns recorded. Reduce entry speed for better cornering precision.',
        color: Color(0xFF4F6B8F),
      ));
    }

    // Speed insight
    final double avgTopSpeed = trips.fold(0.0, (s, t) => s + t.topSpeedKmh) / trips.length;
    if (avgTopSpeed > 100) {
      insights.add(const _InsightCard(
        icon: Icons.speed_rounded,
        title: 'Speed Profile',
        body: 'High average top speeds recorded. Ensure adherence to local track limits.',
        color: Color(0xFF9E653F),
      ));
    }
    ...
    if (insights.isEmpty) {
      insights.add(const _InsightCard(
        ...
        title: 'Outstanding Driver',
        body: 'All metrics look excellent. Keep maintaining this premium driving standard.',
        color: Color(0xFF5A7D65),
      ));
    }
